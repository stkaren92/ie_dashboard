...
text4 <- function(diff_value){
  if(diff_value > 0) {
    "Se detecta un decremento de la superficie de $IE_{alta}$, en la ZI, es deseable que dicha tendencia se revierta y se mantenga bajo monitoreo."
  } else if(diff_value < 0) {
    "Se detecta un incremento de la superficie de $IE_{alta}$ en la ZI, es deseable que dicha tendencia se mantenga en aumento y bajo monitoreo"
  } else {
    "Se detecta un valor similar de la superficie de $IE_{alta}$ en la ZI, es deseable que dicha tendencia se revierta y se mantenga bajo monitoreo"
  }
}
...
## Estado de la Integridad Ecosistémica del ANP ($EIE$)
...
## Eficacia en la protección de la Integridad Ecosistémica del ANP ($Ef_{IE}$)
...
$Ef_{2023}$ de `r anp_shortname`: `r color_efectividad(efectividad_anp)` `r diff_text(efectividad_anp_diff)`.
...
table_change_com %>%
  select(Complejo, ie_alta_diff_symbol, pres_diff_symbol, ef_diff_symbol) %>% 
  ...
  kable(escape = FALSE, align = "c", format = "latex",
        booktabs = T) %>%
  kable_styling(position = "center", latex_options = c("hold_position")) %>% 
  column_spec(2, background = table_change_com %>%
                pull(ie_alta_color) %>%
                purrr::map_chr(~ colores[.x])) %>%
  column_spec(3, background = table_change_com %>%
                pull(pres_diff_color) %>%
                purrr::map_chr(~ colores[.x])) %>%
  column_spec(4, background = table_change_com %>%
                pull(ef_diff_color) %>%
                purrr::map_chr(~ colores[.x])) 
```

$EIE_{2023}$ de `r complejo_name`: `r color_ie_alta(ie_alta_com)` `r diff_text(ie_alta_com_diff)`.

$P_{2023}$ de `r complejo_name`: `r color_presion(presion_com)` `r diff_text(presion_com_diff)`.

$Ef_{2023}$ de `r complejo_name`: `r color_efectividad(efectividad_com)` `r diff_text(efectividad_com_diff)`.

## SÍNTESIS

La ANP `r anp_name` presenta un estado de `r color_ie_alta(ie_alta_anp)`, `r color_presion(presion_anp)` y `r color_efectividad(efectividad_anp)`. Esto significa que `r text1(ie_alta_anp)` de Conservación de IE muy alta y mantener el monitoreo del estado IE del ANP y su Zona de Influencia (ZI).

Dado el estado de conservación de la IE en el ANP y la tendencia descrita entre el 2020 y 2023, es deseable que la tendencia en el estado de la Conservación de la IE `r text2(ie_alta_anp_diff)` y la tendencia de la eficacia `r text2(efectividad_anp_diff)`. Las otras ANP del complejo presentan una `r color_efectividad(efectividad_com)`, es decir que son `r text3(efectividad_anp, efectividad_com)`.
...
Para el año 2023 el $EIE$ del ANP fue de `r ie_alta_anp`% y en el complejo fue de `r ie_alta_com`%. De acuerdo a las clases definidas con base en los criterios, la ANP presenta `r color_ie_alta(ie_alta_anp)` y el complejo al que pertenece `r color_ie_alta(ie_alta_com)`.
...
La ZI del ANP tiene un total de `r 100-presion_anp`% de $IE_{alta}$, por lo que la ANP tiene una `r color_presion(presion_anp)` de `r presion_anp`%. `r text4(efectividad_anp_diff)`.
...
El valor de $Ef_{IE}$ calculado para todas las ANP en el complejo es de `r efectividad_com` y en el ANP `r anp_name` es de `r efectividad_anp`, por lo que se considera que la eficacia es de tipo: `r color_efectividad(efectividad_com)` en el complejo y `r color_efectividad(efectividad_anp)` en el ANP de acuerdo a los criterios definidos. Entre el 2020 y el 2023 se detectó una tendencia `r diff_text(efectividad_anp)` en la eficacia del ANP.
