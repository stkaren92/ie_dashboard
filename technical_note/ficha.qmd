...
editor: visual
params:
  rproject_path: "/Users/kasanchez/Documents/ie_dashboard"
  project_name: "CoSMoS" # CoSMoS or Sierra y Mar
  anp_name: "Iztaccíhuatl-Popocatépetl"
---

```{r define-colorize, echo=FALSE}
color_ie_alta <- function(ie_alta_anp) {
  if (ie_alta_anp < 25) {
    estado <- "Estado de conservación bajo"
    color <- gsub("#", "", col_palette[1])
  } else if (ie_alta_anp <= 50) {
    estado <- "Estado de conservación medio"
    color <- gsub("#", "", col_palette[2])
  } else if (ie_alta_anp <= 75) {
    estado <- "Estado de conservación alto"
    color <- gsub("#", "", col_palette[3])
  } else {
    estado <- "Estado de conservación muy alto"
...
text1 <- function(ie_value){
  if(ie_value <= 50) {
    "hay un grado de preocupación alto en el ANP, por lo que se recomienda alcanzar un estado"
  } else if(ie_value <= 75) {
...
text3 <- function(ef_anp, ef_com){
  ...
  if(diff_value > 0) {
    sprintf("más eficaces que el ANP %s, por lo tanto las acciones en el ANP parecieran ser insuficientes", anp_name)
  } else if(diff_value < 0) {
...
rproject_path <- params$rproject_path

# Input
buffer_folder <- paste0(rproject_path,"/data/Zonas de Influencia_Vectores/")
anp_shp <- vect(paste0(rproject_path,"/data/anp/232_ANP-ITRF08_04072025.shp"))

# Update Cienegas de Lerma shapefile
lerma_shp <- vect(paste0(rproject_path,"/data/anp/cienegas_lerma/Subzon_CLerma_utmitrf08_DOFPM.shp"))
lerma_shp <-  project(lerma_shp, crs(anp_shp))
lerma_shp <- aggregate(lerma_shp)
values(lerma_shp) <- values(anp_shp[anp_shp$NOMBRE == "Ciénegas del Lerma"])
anp_shp <- anp_shp[anp_shp$NOMBRE != "Ciénegas del Lerma"]
anp_shp <- rbind(anp_shp, lerma_shp)


catalogo_anps <- read.csv(paste0(rproject_path,"/data/catalogo_anps.csv"),
...
complejo_name <- df_anps[df_anps$ANP_name == anp_name,"Complejo"]
anp_shortname <- df_anps[df_anps$ANP_name == anp_name,"ANP_shortname"]
...
for(anp in df_anps$ANP_name) {
  for(y in c(2020,2023)){
    ...
    # Get ANP IE raster
    ie <- rast(paste0(rproject_path,'/data/ie/ie_xgb_',y,'.tif'))
    anp_selected_shp <- project(subset(anp_shp, anp_shp$NOMBRE == anp), crs(ie))
    ie_anp <- crop(ie, 
                   anp_selected_shp, 
                   mask=TRUE)
    
    # Get periphery IE raster
    anp_selected_buffer <- df_anps[df_anps$ANP_name == anp,
                                   "Buffer_file"]
...
# IE raster
ie <- rast(paste0(rproject_path,'/data/ie/ie_xgb_2023.tif'))
# ANP shapefile
...
anp_selected_buffer <- df_anps[df_anps$ANP_name == anp_name,
                               "Buffer_file"]
if(nzchar(anp_selected_buffer)){
      if(!anp_selected_buffer %in% names(buffer_list)) {
        stop(paste("No existe el shapefile de zona de influencia:",
...
```{r height_plot, echo=FALSE, message=FALSE, results='hide'}
height_plot <- length(unique(df_dist$ANP_shortname)) * 0.4
```

```{r distribution-plot, echo=FALSE, message=FALSE, results='hide'}
#| fig-height: !expr height_plot
...
data.frame(col = c("Tendencia de indicadores en periodo de 3 años",
                   "= igual al año inicial del periodo",
                   "$\\uparrow$ incremento en el periodo",
                   "$\\downarrow$ decremento en el periodo",
                   "Nota: la tendencia cambia con el cambio de unidad")) %>% 
  kable(escape = FALSE, 
        col.names = NULL,
        align = c("l"), format = "latex",
        booktabs = T) %>%
...
```{r ie-alta-change-table, echo=FALSE}
#| layout-ncol: 2
...
# Log-scale if max value > 10
if(max(df_efectividad$efectividad, na.rm = TRUE) > 10){
  p_ef <- p_ef +
  scale_y_log10() 
} 
...
El ANP `r anp_name` presenta un `r color_ie_alta(ie_alta_anp)` de la Integridad Ecosistémica, `r color_efectividad(efectividad_anp)` y `r color_presion(presion_anp)`. Esto significa que `r text1(ie_alta_anp)` de Conservación muy alto y mantener el monitoreo del estado de IE del ANP y su Zona de Influencia (ZI).

Dado el estado de conservación de la IE en el ANP y la tendencia descrita entre el 2020 y 2023, es deseable que la tendencia en el estado de la Conservación `r text2(ie_alta_anp_diff)` y la tendencia de la eficacia `r text2(efectividad_anp_diff)`. Las otras ANP del complejo presentan una `r color_efectividad(efectividad_com_noANP)`, es decir que son `r text3(efectividad_anp,efectividad_com_noANP)`.
...
**IIE** **4 clases,** donde $IE_{muy baja}$ es 1; $IE_{baja}$ es 2; $IE_{media}$ es 3 y la $IE_{alta}$ es 4 (Sánchez *et al*, 2026). La clase alta ($IE_{alta}$) representa principalmente a la **vegetación primaria** remanente en el país, la IE media la **vegetación secundaria**, la IE baja a la **agricultura** y la IE muy baja a zonas de **asentamientos humanos e infraestructura**. La IE alta es tomada como referencia principal para estimar los indicadores de Estado, Eficacia y Presión del Diagnóstico de Integridad Ecosistémica por ser el valor deseable dominante en las ANP del país.

**IIE 16 clases**, donde 1 es la clase: *Degradación muy severa* a la 16 que es la clase llamada *Éstasis* (Sánchez *et al*, 2026).

**Resolución**: 250x250m

```{r print-agradecimientos, echo=FALSE}
#| results: asis

if(project_name == "CoSMoS"){
  cat("## Agradecimientos\n")
  cat("Estas acciones son posibles gracias al apoyo del proyecto Conservación y Uso Sostenible en Montañas y Sierras (CoSMoS), una iniciativa respaldada por la Cooperación Alemana a través de KfW, implementada en coordinación entre CONANP, CONABIO y FMCN.")
}
```
...
INEGI.2021a. Índice de Hemerobia. Capítulo 5.3.1.3.2.2. En: *Cuentas de los Ecosistemas de México. Resultados del Proyecto* Natural Capital Accounting and Valuation of Ecosystem Services *(*NCAVES*)*. México, INEGI, 258 pp.
