...
table_ie_alta_change <- df_dist %>% 
  ...
  mutate(ie_alta_diff = floor(ie_alta_2023 - ie_alta_2020),
...
table_presion_change <- df_dist %>% 
  ...
  pivot_wider(names_from = "year",
              values_from = "pct",
              names_prefix = "pres_") %>% 
  mutate(pres_diff = floor(pres_2023 - pres_2020),
...
table_efectividad_change <- df_efectividad %>% 
  ...
  pivot_wider(names_from = "year",
              ...
              names_prefix = "ef_") %>%
  mutate(ef_diff = floor(ef_2023 - ef_2020),
...
# Diagnóstico de Integridad Ecosistémica (IE) de la ANP `r anp_name` y su complejo
...
La ANP `r anp_name` presenta un estado de `r color_ie_alta(ie_alta_anp)`, `r color_efectividad(efectividad_anp)` y `r color_presion(presion_anp)`. Esto significa que `r text1(ie_alta_anp)` de Conservación muy alta y mantener el monitoreo del estado de IE del ANP y su Zona de Influencia (ZI).
...
**Lo deseable en una ANP es tener 100% de su área con clase** **$IE_{alta}$**.
